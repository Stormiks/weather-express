...
from hdc1080 import hdc1080_constant
import hdc1080.i2c as I2C
...
reg_list = {'TEMP': hdc1080_constant.TEMP,
            'HUMDT':  hdc1080_constant.HUMDT,
            'CONF': hdc1080_constant.CONF,
            'SER_ID1': hdc1080_constant.SER_ID1,
            'SER_ID2': hdc1080_constant.SER_ID2,
            'SER_ID3': hdc1080_constant.SER_ID3,
            'MANUF': hdc1080_constant.MANUF,
            'DEVID': hdc1080_constant.DEVID,
            }
...
def register_list():
    ...
    hdc = HDC1080()
    hdc._logger = logging.getLogger('hdc1080.reglist')
    register = {}
    ...
    reg_conf['HRES'] = hres_switch.get((hdc.read_register(register='CONF')[
                                       0] & conf_mask_bit_list['CONF_HRES']))
    reg_conf['TRES'] = tres_switch.get((hdc.read_register(register='CONF')[
                                       0] & conf_mask_bit_list['CONF_TRES']))
    reg_conf['BAT'] = 'LOW' if hdc.read_register(
        register='CONF')[0] & conf_mask_bit_list['CONF_BAT'] > 0 else 'GOOD'
    reg_conf['MODE'] = 'BOTH' if hdc.read_register(
        register='CONF')[0] & conf_mask_bit_list['CONF_MODE'] > 0 else 'ONLY'
    reg_conf['HEAT'] = 'ENABLE' if hdc.read_register(
        register='CONF')[0] & conf_mask_bit_list['CONF_HEAT'] > 0 else 'DISABLE'
    ...
    def read_register(self, register):
        if register == 'TEMP' or register == 'HUMDT' or register == 'CONF' or register == 'SER_ID1' or register == 'SER_ID2' or register == 'SER_ID3' or register == 'MANUF' or register == 'DEVID':
            ...
            try:
                ...
                reg_status_hex = '0x' + \
                    '{0:02x}'.format(
                        reg_status_bita[0]) + '{0:02x}'.format(reg_status_bita[1])
    ...
    def write_register(self, register, bits):
        ...
        if register == 'CONF':
            for ibit in bits:
                ...
                self._logger.debug('write_register, init reg_status: %s, bit_mask_reg %s', '{0:04X}'.format(
                    reg_status), format(bit_clr))
                reg_status = reg_status | conf_bit_on_list[ibit]
                self._logger.debug('write_register, init reg_status: %s, bit %s', '{0:04X}'.format(
                    reg_status), '{0:04X}'.format(conf_bit_on_list[ibit]))
        ...
        if ret > 1:
            ...
            self._logger.debug(
                'write_register %s, byte data: %s', register, byt_reg)
            self._logger.debug('write_register %s, data: %s',
                               register, '{0:04X}'.format(reg_status))
        return ret
